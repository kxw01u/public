...
COL_LAST_UPDATE, COL_IDLE, COL_PIC, COL_REMARK = 11, 12, 13, 14  # COL_REMARK = Dep
COL_STEP_START = 15
    ...
    def paint(self, painter, option, index):
        ...
        if bg:
            ...
            if col == project_col:
                ...
                ptxt = item.text(priority_col).strip() if item else ""
...
class RecordKeeper(QWidget):
    ...
    def setup_row_widgets(self, it):
        ...
        pb.setStyleSheet("""
            QProgressBar { border: none; text-align: center; margin: 0; padding: 0; min-height: 28px; max-height: 28px; }
            QProgressBar::chunk { margin: 0; border-radius: 3px; background-color: #3b82f6; }
        """)
    ...
    def add_child(self, parent):
        ...
        self.update_parent_stage_from_children(parent, recurse_up=True)
    ...
    def lock_parent_steps(self, parent):
        for c in range(COL_STEP_START, COL_STEP_END + 1):
            ...
            parent.setTextAlignment(c, Qt.AlignmentFlag.AlignCenter)

    def unlock_parent_steps(self, parent):
        for c in range(COL_STEP_START, COL_STEP_END + 1):
            parent.setFlags(parent.flags() | Qt.ItemFlag.ItemIsEditable)
            parent.setBackground(c, QColor("#ffffff"))
            parent.setTextAlignment(c, Qt.AlignmentFlag.AlignCenter)

    def update_parent_stage_from_children(self, parent, recurse_up=False):
        if parent.childCount() == 0:
            return
        for c in range(COL_STEP_START, COL_STEP_END + 1):
            statuses = [parent.child(i).text(c) for i in range(parent.childCount())]
            if all(s in ("Done", "N/A") for s in statuses if s != "") and any(s in ("Done", "N/A") for s in statuses):
                parent.setText(c, "Done")
                self.color_step_cell(parent, c, "Done")
            elif all(s in ("", "WIP") for s in statuses):
                parent.setText(c, "")
        ...
        if recurse_up and parent.parent():
            self.update_parent_stage_from_children(parent.parent(), recurse_up=True)

    def del_item(self, it):
        par = it.parent()
        if par:
            par.removeChild(it)
            self.mark_bold(par, par.childCount() > 0)
            if par.childCount() == 0:
                self.unlock_parent_steps(par)
            else:
                self.update_parent_stage_from_children(par, recurse_up=True)
        else:
    ...
    def on_click(self, it, col):
        if col == COL_COLOR:
            ...
            cb = QComboBox(); cb.addItems(NATURES)
            cur = it.text(COL_NATURE).strip()
            if cur in NATURES: cb.setCurrentText(cur)
            cb.activated.connect(lambda _ix, i=it, w=cb: self.commit_combo(i, COL_NATURE, w))
            self.tree.setItemWidget(it, COL_NATURE, cb); cb.showPopup()

        elif col == COL_PRIORITY:
            cb = QComboBox(); cb.addItems(["1","2","3","4"])
            cur = it.text(COL_PRIORITY).strip()
            if cur in {"1","2","3","4"}: cb.setCurrentText(cur)
            cb.activated.connect(lambda _ix, i=it, w=cb: self.commit_priority(i, w))
            self.tree.setItemWidget(it, COL_PRIORITY, cb); cb.showPopup()
            ...
            if cur and cur in pics: cb.setCurrentText(cur)
            cb.activated.connect(lambda _ix, i=it, w=cb: self.commit_combo(i, COL_PIC, w))
            self.tree.setItemWidget(it, COL_PIC, cb); cb.showPopup()

        elif col == COL_REMARK:  # Dep
            cb = QComboBox()
            cases = []
            for it2 in self.iterate_items():
                cnum = it2.data(COL_PROJECT, ROLE_CASE)
                if cnum: cases.append(cnum)
            cb.addItems(sorted(set(cases)))
            cur = it.text(COL_REMARK).strip()
            if cur in cases: cb.setCurrentText(cur)
            cb.activated.connect(lambda _ix, i=it, w=cb: self.commit_combo(i, COL_REMARK, w))
            self.tree.setItemWidget(it, COL_REMARK, cb); cb.showPopup()

    def on_right_click_step(self, it, col):
        if not (COL_STEP_START <= col <= COL_STEP_END): return
        cur = it.text(col)
    ...
    def update_progress(self, it):
        ...
        if pb:
            ...
            if pct >= 100:
                pb.setStyleSheet("""
                    QProgressBar { border: none; text-align: center; margin: 0; padding: 0; min-height: 28px; max-height: 28px; }
                    QProgressBar::chunk { margin: 0; border-radius: 3px; background-color: #16a34a; }
                """)
            else:
                pb.setStyleSheet("""
                    QProgressBar { border: none; text-align: center; margin: 0; padding: 0; min-height: 28px; max-height: 28px; }
    ...
    def enforce_parent_rules_all(self):
        for it in self.iterate_items():
            if it.childCount() > 0:
                self.lock_parent_steps(it)
        for it in self.iterate_items():
            if it.childCount() > 0:
                self.update_parent_stage_from_children(it, recurse_up=False)
